...
class FeatureExploration:
    def __init__(self, df=None,config=None,base_path=''):
        
        if df is None:
            temp_df_path = os.path.join(base_path,config.get("output_folder","output"),'Prep', 'prepared_data.csv') 
            print(f"Reading prepared_data from {temp_df_path}") # Load the processed data
        ...
        self.output_folder = os.path.join(base_path,self.config.get('output_folder', 'output'),'Explore')
    ...
    def select_features_based_on_importance(self, importance_df):
        ...
        if top_n:
            ...
            selected_features.to_csv(f"{self.output_folder}/selected_features_top_{top_n}.csv", index=False)
            print(f"Top {top_n} features selected and saved to {self.output_folder}/selected_features_top_{top_n}.csv")
