...
class Scoring:
    def __init__(self, config, base_path='', logger=None):
        """
        ...
        self.output_folder = os.path.join(base_path,config.get("output_folder", "output"))
        ...
        self.score_folder = os.path.join(base_path,config.get("output_folder", "output"),"Score")
    ...
    def score(self, oot_data):
        ...
        try:
            ...
            if self.config['data_preparation']['enabled']=='Y':
                ...
                self.logger.info("Prepared data saved to data/oot_prepared_data.csv")
            ...
            eval_result_file = os.path.join(self.score_folder,"eval_results.csv")
            pd.DataFrame([evaluation_results]).to_csv(eval_result_file,index=False)
